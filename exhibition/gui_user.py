...
class LatentDreamscapeGUI:
    ...
    def setup_gui(self):
        ...
        # Mode switch button
        self.mode_button = tk.Button(self.main_frame, text="Switch to Drawing Mode",
                                     command=self.switch_mode, font=("Courier", 12))
        self.mode_button.grid(row=0, column=0, columnspan=2, pady=5)
        ...
        self.text_entry = tk.Text(self.text_frame, width=60, height=8, wrap="word",
        ...
        # EEG Status Section
        self.eeg_label = tk.Label(self.main_frame, text="EEG Status", font=("Courier", 14), bg="#f0f0f0")
        self.eeg_label.grid(row=2, column=0, pady=(20, 0))
        self.eeg_status_canvas = tk.Canvas(self.main_frame, width=60, height=60, bd=0, highlightthickness=0)
        self.eeg_status_canvas.grid(row=3, column=0, pady=5)
    ...
    def switch_mode(self):
        """Switch between Text and Drawing modes and send OSC mode."""
        if self.mode == "text":
            self.mode = "drawing"
            ...
            self.mode_button.config(text="Switch to Drawing Mode")

        # Send OSC message for current mode
        mode_value = self.mode.encode()
        self.client.send_message(b"/mode", [mode_value])
        print(f"Mode sent over OSC: {self.mode}")
